...
url = "https://climate.nasa.gov/effects/"  # link of webpage
headers = {
...
ari = (4.71 * (len(characters)/(len(words)) + 0.5 * (len(words)) /
       (len(sentences))))  # Formula of Automated Readability Index (ARI)
print("Fog Index Value : ", ari)

# Formula of average Number of Words per sentence.
avgword = (len(words)/(len(sentences)))
